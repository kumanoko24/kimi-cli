...
- **codex**: OpenAI Codex CLI — good for focused, one-shot code tasks. Runs `codex exec` non-interactively. Supports `model_id` (e.g. `gpt-5.5`, `codex-mini-latest`) and `effort` (`low`/`medium`/`high`/`xhigh` reasoning effort).
